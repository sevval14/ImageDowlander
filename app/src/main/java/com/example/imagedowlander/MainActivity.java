package com.example.imagedowlander;
...
//         });
    ...
    class DownloadTask extends AsyncTask<String, Integer, Bitmap> {
        ...
        protected Bitmap doInBackground(String... strings) {
            ...
            String imagePath = (Environment.getExternalStorageState(new File(Environment.DIRECTORY_DOWNLOADS + "/" + fileName)));
            downloadFile(strings[0], imagePath + "/" + fileName);
            return Bitmap.createScaledBitmap(imagePath + "/" + fileName);
        }
    ...
    private Bitmap scaledBitmep(String imagePath) {
        Bitmap image = BitmapFactory.decodeFile(imagePath);
        float w = image.getWidth();
        float h = image.getHeight();
        int W = 400;
